...
import RawMaterialsPageEditor from '@/app/admin/pages/RawMaterialsPageEditor';
                ...
                >
                    ...
                    <div className="flex flex-wrap gap-3">
                        ...
                        >
                            ...
                            {t('ui-labels-open-datasheet', language === 'ar' ? 'فتح ورقة البيانات' : 'Open Datasheet')}
                        </a>
                        <a
                            href={sheet.pdfUrl}
                            target="_blank"
                        ...
                        >
                            ...
                            {t('ui-labels-download-pdf', language === 'ar' ? 'تحميل PDF' : 'Download PDF')}
                        </a>
                    </div>
                </div>
            </div>
...
export default function RawMaterialSupply() {
        ...
        try {
            ...
            if (newContentAr) {
                Object.keys(newContentAr).forEach(key => {
                    if (newContentAr[key] !== contentAr[key] && idMap[key]) {
                        ...
                        if (existingUpdate) {
                            ...
                            updates.push({
                                id: idMap[key],
                                value: content[key], // Keep existing English
    ...
    useEffect(() => {
        const loadContent = async () => {
            try {
                ...
                setRawContent(data); // Save raw content for editor
                ...
                Object.keys(data).forEach(section => {
                    Object.keys(data[section]).forEach(key => {
                        ...
                        // All other sections (material-N, sheet-N, materials-section, datasheets-section, ui-labels) need prefix
                        const fullKey = (section === 'hero' || section === 'cta')
                            ? key
                            : `${section}-${key}`;

                        flat[fullKey] = data[section][key].value;
                        if (data[section][key].valueAr) {
                            flatAr[fullKey] = data[section][key].valueAr;
                        }
                        ids[fullKey] = data[section][key].id;
                    });
                });
                setContent(flat);
                setContentAr(flatAr);
    ...
    const displayMaterials = initialMaterials.map((m, i) => ({
        ...m,
        title: t(`material-${i + 1}-title`, m.title),
        image: t(`material-${i + 1}-image`, m.image),
        polymer: t(`material-${i + 1}-polymer`, m.polymer),
        source: t(`material-${i + 1}-source`, m.source),
        color: t(`material-${i + 1}-color`, m.color),
        additives: t(`material-${i + 1}-additives`, m.additives),
        link: t(`material-${i + 1}-link`, m.link),
    }));

    const displayDataSheets = initialDataSheets.map((s, i) => ({
        ...s,
        title: t(`sheet-${i + 1}-title`, s.title),
        subtitle: t(`sheet-${i + 1}-subtitle`, s.subtitle),
        badges: (language === 'ar' && contentAr[`sheet-${i + 1}-badges`]) ? contentAr[`sheet-${i + 1}-badges`].split(',').map(b => b.trim()) : (content[`sheet-${i + 1}-badges`] ? content[`sheet-${i + 1}-badges`].split(',').map(b => b.trim()) : s.badges),
        description: t(`sheet-${i + 1}-description`, s.description),
    ...
    return (
        <div className="min-h-screen" style={{ background: isLight ? '#F8FBFF' : '#0B1121' }}>
            ...
            {isEditing && (
                <RawMaterialsPageEditor
                    content={Object.keys(content).length > 0 ?
                        Object.keys(content).reduce((acc, key) => {
                            acc[key] = {
                                value: content[key],
                                valueAr: contentAr[key] || '',
                                id: idMap[key] || 0
                            };
                            return acc;
                    ...
                    <div className="grid md:grid-cols-2 gap-12 items-center">
                        ...
                        <div>
                            ...
                            >
                                ...
                                >
                                    {t('badge-text', 'Raw Material Supply')}
                                </span>
                            </div>
                            ...
                            >
                                <span style={{ color: isLight ? '#0F172A' : '#fff', whiteSpace: 'pre-line' }}>
                                    {t('title', 'Sustainable').replace(/Sustainable/i, 'Sustainable')}
                                </span>
                                {!content['title'] && !contentAr['title'] && <br />}
                                <span
                                    style={{
                                ...
                                >
                                    {!content['title'] && !contentAr['title'] && 'Materials'}
                                </span>
                            </h1>
                            ...
                            >
                                {t('description', 'Our commitment to sustainability drives innovation in polymer technology. We develop advanced materials that minimize environmental impact while delivering superior performance. Explore our eco-conscious product portfolio designed for a greener future.')}
                            </p>
                        </div>
                        ...
                        >
                            {[
                                ...
                                ...stat,
                                value: t(`stat-${i + 1}-value`, stat.value),
                                label: t(`stat-${i + 1}-label`, stat.label),
                            })).map((stat, i) => (
                                <div
                                    key={i}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    >
                        <h2 className="text-3xl lg:text-4xl font-bold mb-3" style={{ color: isLight ? '#0F172A' : '#fff' }}>
                            {language === 'ar' ? (
                                t('materials-section-title', 'موادنا')
                            ) : (
                                <>
                                    Our{' '}
                                    <span
                        ...
                        <p className="text-base" style={{ color: isLight ? '#94A3B8' : 'rgba(255,255,255,0.45)' }}>
                            {t('materials-section-subtitle', 'Discover our range of sustainable polymer solutions')}
                        </p>
                    </div>
            ...
            <section ref={sheetsRef} className="py-16 lg:py-24" style={{ background: isLight ? '#F8FBFF' : '#0B1121' }}>
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    >
                        <h2 className="text-3xl lg:text-4xl font-bold mb-3" style={{ color: isLight ? '#0F172A' : '#fff' }}>
                            {language === 'ar' ? (
                                t('datasheets-section-title', 'أوراق بيانات المنتج')
                            ) : (
                                <>
                                    Product{' '}
                                    <span
                        ...
                        <p className="text-base" style={{ color: isLight ? '#94A3B8' : 'rgba(255,255,255,0.45)' }}>
                            {t('datasheets-section-subtitle', 'Explore our comprehensive range of sustainable polymer materials')}
                        </p>
                    </div>
            ...
            <section className="py-20 relative overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
                    ...
                    >
                        {t('cta-title', language === 'ar' ? 'هل تحتاج مواد مخصصة؟' : 'Need Custom Materials?')}
                    </h2>
                    <p className="text-xl mb-10" style={{ color: isLight ? '#64748B' : 'rgba(255,255,255,0.6)' }}>
                        {t('cta-description', language === 'ar' ? 'اتصل بفريق المواد لدينا للحصول على تركيبات بوليمر مخصصة ودعم فني' : 'Contact our materials team for custom polymer formulations and technical support')}
                    </p>
                    <div className="flex flex-col sm:flex-row justify-center gap-4">
                        ...
                        >
                            {t('button-sample', language === 'ar' ? 'اطلب عينة ←' : 'Request a Sample →')}
                        </a>
                        <a
                            href="/contact"
                        ...
                        >
                            {t('button-contact', language === 'ar' ? 'تواصل مع المبيعات' : 'Contact Sales')}
                        </a>
                    </div>
                </div>
            </section>
